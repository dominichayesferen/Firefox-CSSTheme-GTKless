...
success=0
for directory in ~/.mozilla/firefox ~/.var/app/org.mozilla.firefox/.mozilla/firefox ~/.var/app/org.mozilla.FirefoxNightly/.mozilla/firefox ~/.librewolf ~/.var/app/io.gitlab.librewolf-community/.librewolf ~/.floorp ~/.var/app/one.ablaze.floorp/.floorp ~/.waterfox ~/.var/app/net.waterfox.waterfox/.waterfox ~/.var/app/net.mullvad.MullvadBrowser/.mullvad; do
    if [ -d $directory ]; then
        cd $directory
